...
namespace SmartTracking.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //[ValidateAntiForgeryToken]
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult> Register(RegisterViewModel model, int[] selectedProjects)
        {
            ...
            //    {
            //        var projectToAdd = db.Projects.Find(project);
            //        model.Projects.Add(projectToAdd);
            //    }
            //}

            //if (ModelState.IsValid)
            //{
            //    var user = new ApplicationUser { UserName = model.UserName, FirstName = model.FirstName, LastName = model.LastName, DisplayName = model.DisplayName, BugNetUserId = model.BugNetUserId, Email = model.Email };
            //    db.Users.Add(user);
            //    db.SaveChanges();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    string _userId = db.Users.FirstOrDefault(m => m.UserName == user.UserName).Id;
                    if (selectedProjects != null)
                    {
                        foreach (var project in selectedProjects)
                        {
                            ...
                            _projectUser.UserId = _userId;
                            db.ProjectUsers.Add(_projectUser);
                            db.SaveChanges();
                        }
                    }
